    ...
    async def sync_to_schedule(
        ...
        for assignment in assignments:
            try:
                ...
                result.add_error(f"Assignment {assignment.name}: {e}")
        ...
        for event in events:
            ...
            try:
                ...
                logger.error(f"Failed to sync calendar event {event.id}: {e}")
                result.add_error(f"Event {event.title}: {e}")
    ...
    async def _create_schedule_event(
        ...
        if self.event_creator:
            try:
                ...
                if event_id:
                    logger.info(f"Created schedule event: {event_data['title']}")
                return event_id
            except Exception as e:
            ...
            # 没有回调，只记录日志
            logger.debug(f"Event data ready (no creator): {event_data['title']}")
            return None
